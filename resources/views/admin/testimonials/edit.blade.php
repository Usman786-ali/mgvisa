...
<div style="padding: 2rem; max-width: 800px; margin: 0 auto;">
    <div style="margin-bottom: 2rem;">
        ...
        <h1 style="font-size: 2rem; font-weight: 700; color: #1f2937;">Edit Testimonial</h1>
    </div>
    ...
    <div style="background: white; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.08); padding: 2rem;">
        <form action="{{ route('admin.testimonials.update', $testimonial) }}" method="POST">
                ...
                <div style="display: flex; gap: 1rem;">
                    @for($i = 1; $i <= 5; $i++)
                        <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                            <input type="radio" name="rating" value="{{ $i }}" {{ old('rating', $testimonial->rating) == $i ? 'checked' : '' }} required>
            ...
            <div style="margin-bottom: 1.5rem;">
                <label style="display: block; margin-bottom: 0.5rem; font-weight: 600; color: #374151;">
                    Testimonial Message <span style="color: #ef4444;">*</span>
                </label>
                <textarea name="message" rows="5" required
                          placeholder="Write the client's testimonial here..."
                          style="width: 100%; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 8px; font-size: 1rem; font-family: inherit;">{{ old('message', $testimonial->message) }}</textarea>
                @error('message')
                    <p style="color: #ef4444; font-size: 0.875rem; margin-top: 0.25rem;">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div style="margin-bottom: 2rem;">
                <label style="display: flex; align-items: center; gap: 0.75rem; cursor: pointer;">
                    ...
                    <span style="font-weight: 600; color: #374151;">
                        <i class="fas fa-star" style="color: #F59E0B;"></i> Feature this testimonial on homepage
                    </span>
                </label>
            </div>
