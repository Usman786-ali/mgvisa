...
@section('content')
    <div style="padding: 2rem;">
        <div style="margin-bottom: 2rem; display: flex; justify-content: space-between; align-items: center;">
            ...
            <a href="{{ route('admin.packages.create') }}" style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); color: white; padding: 0.75rem 1.5rem; border-radius: 8px; text-decoration: none; font-weight: 600; display: inline-flex; align-items: center; gap: 0.5rem; box-shadow: 0 4px 12px rgba(16, 185, 129, 0.3);">
        ...
        <div style="display: grid; gap: 1.5rem;">
            @foreach($packages as $package)
                ...
                @endphp
                <div style="background: white; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.08); padding: 2rem; position: relative; {{ $isRamadan ? 'border: 2px solid #10b981;' : ($package->type == 'gold' ? 'border: 2px solid #B8941F;' : '') }}">
                    @if($package->is_popular)
                        <div style="position: absolute; top: -12px; right: 20px; background: linear-gradient(135deg, {{ $isRamadan ? '#059669, #10b981' : '#B8941F, #9C7A1A' }} ); color: white; padding: 0.5rem 1rem; border-radius: 20px; font-weight: 600; font-size: 0.875rem; box-shadow: 0 4px 8px rgba(0,0,0,0.2);">
                            <i class="fas fa-star"></i> {{ $isRamadan ? 'Ramadan Special' : 'Best Value' }}
                        </div>
                    @endif
                    
                    @if($isRamadan && !$package->is_popular)
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr 1fr auto; gap: 2rem; align-items: start;">
                        ...
                        <div>
                            <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1rem;">
                                <div style="width: 60px; height: 60px; background: {{ $isRamadan ? '#dcace0' : ($package->type == 'basic' ? '#f3f4f6' : ($package->type == 'gold' ? '#fef3c7' : '#f5f3ff')) }}; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; color: {{ $isRamadan ? '#059669' : ($package->type == 'basic' ? '#6b7280' : ($package->type == 'gold' ? '#B8941F' : '#7c3aed')) }};">
                                    <i class="{{ $package->icon }}"></i>
                                </div>
                                <div>
                                    <h3 style="font-size: 1.5rem; font-weight: 700; color: #1f2937; margin: 0;">{{ $package->name }}</h3>
                                    <p style="color: #6b7280; margin: 0.25rem 0 0 0;">{{ $package->duration }}</p>
                                    <span style="display: inline-block; background: #eee; padding: 2px 8px; border-radius: 4px; font-size: 0.75rem; color: #555; margin-top: 4px;">{{ ucfirst($package->category) }} - {{ $package->type }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div style="font-size: 2rem; font-weight: 700; color: #B8941F;">PKR {{ number_format($package->price, 0) }}</div>
                            ...
                            <div style="display: flex; flex-direction: column; gap: 0.25rem;">
                                @foreach($package->features as $feature)
                                    <div style="display: flex; align-items: center; gap: 0.5rem; font-size: 0.875rem;">
                                        ...
                                        @endif
                                        <span style="{{ !$feature['included'] ? 'opacity: 0.5;' : '' }}">{{ $feature['text'] }}</span>
                                    </div>
                                @endforeach
                            </div>
                        </div>
