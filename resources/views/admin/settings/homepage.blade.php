...
@section('content')
    <div style="padding: 2rem;">
        ...
        <form action="{{ route('admin.settings.updateHomepage') }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            @if(isset($settings['homepage_hero']))
                <div
                    ...
                    <div style="display: grid; gap: 1.5rem;">
                        @foreach($settings['homepage_hero'] as $setting)
                            @if($setting->key == 'hero_media_type')
                                <!-- Media Type Selector -->
                                <div style="background: #f9fafb; padding: 1.5rem; border-radius: 8px; border: 2px dashed #B8941F;">
                                    ...
                                    <div style="display: flex; gap: 1rem; flex-wrap: wrap;">
                                        <label
                                            ...
                                            <input type="radio" name="{{ $setting->key }}" value="image" {{ $setting->value == 'image' ? 'checked' : '' }} onclick="updateMediaUpload('image')">
                                            <i class="fas fa-image" style="color: #3b82f6;"></i>
                                            <span style="font-weight: 600;">Single Image</span>
                                        </label>
                                        <label
                                            style="display: flex; align-items: center; gap: 0.5rem; padding: 0.75rem 1.25rem; background: white; border: 2px solid #d1d5db; border-radius: 8px; cursor: pointer; transition: all 0.3s;">
                                            <input type="radio" name="{{ $setting->key }}" value="video" {{ $setting->value == 'video' ? 'checked' : '' }} onclick="updateMediaUpload('video')">
                                            <i class="fas fa-video" style="color: #ef4444;"></i>
                                            <span style="font-weight: 600;">Video</span>
                                        </label>
                                        <label
                                            style="display: flex; align-items: center; gap: 0.5rem; padding: 0.75rem 1.25rem; background: white; border: 2px solid #d1d5db; border-radius: 8px; cursor: pointer; transition: all 0.3s;">
                                            <input type="radio" name="{{ $setting->key }}" value="slideshow" {{ $setting->value == 'slideshow' ? 'checked' : '' }}
                                ...
                                <div id="mediaUploadField"
                                    ...
                                    @if($setting->value)
                                        <div style="margin-bottom: 1rem; padding: 1rem; background: white; border-radius: 8px;">
                                            ...
                                            @if($mediaType == 'video')
                                                <video src="{{ asset($mediaValue) }}" controls
                                            ...
                                            @elseif($mediaType == 'slideshow')
                                                ...
                                                <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
                                                    @foreach($images as $img)
                                                        <img src="{{ asset($img) }}" alt="Slide"
                                                            style="width: 100px; height: 100px; object-fit: cover; border-radius: 8px;">
                                                    @endforeach
                                                </div>
                                            @else
                                                <img src="{{ asset($mediaValue) }}" alt="Hero" style="max-width: 300px; border-radius: 8px;">
                                            @endif
                                        </div>
                                    @endif

                                    <!-- File Input -->
                                    <input type="file" id="heroMediaInput" name="{{ $setting->key }}" accept="image/*,video/*" multiple
                            ...
                            @elseif($setting->type == 'textarea')
                                <div>
                                    ...
                                    <textarea name="{{ $setting->key }}" rows="3"
                                        style="width: 100%; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 8px; font-family: inherit;">{{ $setting->value }}</textarea>
                                </div>
            ...
            <!-- About Section -->
            @if(isset($settings['homepage_about']))
                <div
                    style="background: white; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.08); padding: 2rem; margin-bottom: 2rem;">
                    <h2
                        style="font-size: 1.5rem; font-weight: 700; color: #1f2937; margin-bottom: 1.5rem; border-bottom: 2px solid #B8941F; padding-bottom: 0.75rem;">
                    ...
                    <div style="display: grid; gap: 1.5rem;">
                        @foreach($settings['homepage_about'] as $setting)
                            <div>
                                <label
                                    style="display: block; margin-bottom: 0.5rem; font-weight: 600; color: #374151; text-transform: capitalize;">
                                    {{ str_replace('_', ' ', str_replace('about_', '', $setting->key)) }}
                                </label>
                                ...
                                @elseif($setting->type == 'image')
                                    <div style="display: flex; align-items: center; gap: 1rem;">
                                        @if($setting->value)
                                            <img src="{{ asset($setting->value) }}" alt="Current"
                                                style="width: 100px; height: 100px; object-fit: cover; border-radius: 8px;">
                                        @endif
            ...
            <!-- Stats Section -->
            @if(isset($settings['homepage_stats']))
                <div
                    style="background: white; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.08); padding: 2rem; margin-bottom: 2rem;">
                    <h2
                        style="font-size: 1.5rem; font-weight: 700; color: #1f2937; margin-bottom: 1.5rem; border-bottom: 2px solid #B8941F; padding-bottom: 0.75rem;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1.5rem;">
                        @foreach($settings['homepage_stats'] as $setting)
                            <div>
                                <label
                                    style="display: block; margin-bottom: 0.5rem; font-weight: 600; color: #374151; text-transform: capitalize;">
                                    {{ str_replace('_', ' ', str_replace('stats_', '', $setting->key)) }}
                                </label>
            ...
            @foreach(['homepage_services', 'homepage_why', 'homepage_packages', 'homepage_process', 'homepage_contact'] as $group)
                @if(isset($settings[$group]))
                    <div
                        ...
                        <h2
                            ...
                            <i class="fas fa-edit"></i> {{ str_replace('_', ' ', str_replace('homepage_', '', $group)) }} Section
                        </h2>
